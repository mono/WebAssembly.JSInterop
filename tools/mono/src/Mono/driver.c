...
//JS funcs
extern MonoObject* mono_wasm_invoke_js_with_args (int js_handle, MonoString *method, MonoArray *args, int is_managed, int *is_exception);
...
const char* mono_type_get_name (MonoType *type);
...
mono_wasm_load_runtime (const char *managed_path, int enable_debugging)
{
	...
	mono_add_internal_call ("WebAssembly.JSInterop.InternalCalls::InvokeJS", mono_wasm_invoke_js);
	mono_add_internal_call ("WebAssembly.JSInterop.InternalCalls::InvokeJSWithArgs", mono_wasm_invoke_js_with_args);
...
mono_wasm_get_obj_type (MonoObject *obj)
{
	...
	DBG("mono_wasm_get_obj_type %d",mono_type_get_type (type));
...
EMSCRIPTEN_KEEPALIVE int
mono_wasm_is_byte_array (MonoObject *obj)
{
	DBG("mono_wasm_is_byte_array");
...
EMSCRIPTEN_KEEPALIVE MonoArray*
mono_wasm_byte_array_new (char *arr, int length)
{
	DBG("mono_wasm_byte_array_new %i",length);
	MonoArray *buffer;
	buffer = mono_array_new (root_domain, mono_get_byte_class (), length);
	memcpy(mono_array_addr_with_size(buffer, sizeof(char), 0), arr, mono_array_length(buffer));
	return buffer;
}
...
mono_wasm_array_to_heap (MonoArray *src, char *dest)
{
	...
	DBG("mono_wasm_to_heap element size %i  / length %i",element_size, mono_array_length(src));
